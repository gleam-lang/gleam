...
-spec optional(fun((dynamic()) -> {ok, CPW} | {error, list(decode_error())})) -> fun((dynamic()) -> {ok,
        gleam@option:option(CPW)} |
    {error, list(decode_error())}).
optional(Decode) ->
    fun(Value) -> gleam_stdlib:decode_option(Value, Decode) end.

-spec result(
    fun((dynamic()) -> {ok, CPK} | {error, list(decode_error())}),
    fun((dynamic()) -> {ok, CPM} | {error, list(decode_error())})
) -> fun((dynamic()) -> {ok, {ok, CPK} | {error, CPM}} |
    {error, list(decode_error())}).
...
-spec any(list(fun((dynamic()) -> {ok, CSV} | {error, list(decode_error())}))) -> fun((dynamic()) -> {ok,
        CSV} |
    {error, list(decode_error())}).
any(Decoders) ->
...
-spec map_errors(
    {ok, COU} | {error, list(decode_error())},
...
-spec list(fun((dynamic()) -> {ok, CPR} | {error, list(decode_error())})) -> fun((dynamic()) -> {ok,
        list(CPR)} |
    {error, list(decode_error())}).
list(Decoder_type) ->
    fun(Dynamic) ->
...
-spec optional_field(
    any(),
    fun((dynamic()) -> {ok, CQF} | {error, list(decode_error())})
) -> fun((dynamic()) -> {ok, gleam@option:option(CQF)} |
    {error, list(decode_error())}).
...
-spec tuple3(
    fun((dynamic()) -> {ok, CRA} | {error, list(decode_error())}),
...
) -> fun((dynamic()) -> {ok, {CRH, CRJ, CRL, CRN}} |
    {error, list(decode_error())}).
...
) -> fun((dynamic()) -> {ok, {CRQ, CRS, CRU, CRW, CRY}} |
    {error, list(decode_error())}).
...
) -> fun((dynamic()) -> {ok, {CSB, CSD, CSF, CSH, CSJ, CSL}} |
    {error, list(decode_error())}).
...
) -> fun((dynamic()) -> {ok, gleam@map:map_(CSO, CSQ)} |
    {error, list(decode_error())}).
...
-spec decode3(
    fun((CTJ, CTK, CTL) -> CTM),
    fun((dynamic()) -> {ok, CTJ} | {error, list(decode_error())}),
...
-spec decode6(
    fun((CUN, CUO, CUP, CUQ, CUR, CUS) -> CUT),
...
-spec decode7(
    fun((CVB, CVC, CVD, CVE, CVF, CVG, CVH) -> CVI),
    ...
    fun((dynamic()) -> {ok, CVG} | {error, list(decode_error())}),
...
-spec decode8(
    fun((CVR, CVS, CVT, CVU, CVV, CVW, CVX, CVY) -> CVZ),
...
-spec decode9(
    fun((CWJ, CWK, CWL, CWM, CWN, CWO, CWP, CWQ, CWR) -> CWS),
    fun((dynamic()) -> {ok, CWJ} | {error, list(decode_error())}),
    ...
    fun((dynamic()) -> {ok, CWP} | {error, list(decode_error())}),
