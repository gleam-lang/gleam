...
-opaque task(FFD) :: {task,
        gleam@erlang@process:pid_(),
        gleam@erlang@process:pid_(),
        gleam@erlang@process:process_monitor(),
        gleam@erlang@process:selector(message(FFD))}.

-type await_error() :: timeout | {exit, gleam@dynamic:dynamic()}.

-type message(FFE) :: {from_monitor, gleam@erlang@process:process_down()} |
    {from_subject, FFE}.

-spec async(fun(() -> FFF)) -> task(FFF).
async(Work) ->
    Owner = erlang:self(),
...
-spec try_await(task(FFJ), integer()) -> {ok, FFJ} | {error, await_error()}.
...
-spec await(task(FFN), integer()) -> FFN.
await(Task, Timeout) ->
...
-spec try_await_forever(task(FFP)) -> {ok, FFP} | {error, await_error()}.
...
-spec await_forever(task(FFT)) -> FFT.
await_forever(Task) ->
