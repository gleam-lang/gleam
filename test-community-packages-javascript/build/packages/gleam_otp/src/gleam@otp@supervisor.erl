...
-export([supervisor/1, worker/1, returning/2, start_spec/1, start/1, to_erlang_start_result/1, application_stopped/0, add/2]).
...
-type spec(FRW, FRX) :: {spec,
        FRW,
        integer(),
        integer(),
        fun((children(FRW)) -> children(FRX))}.

-opaque children(FRY) :: {ready, starter(FRY)} | {failed, child_start_error()}.

-opaque child_spec(FRZ, FSA, FSB) :: {child_spec,
        fun((FSA) -> {ok, gleam@erlang@process:subject(FRZ)} |
            {error, gleam@otp@actor:start_error()}),
        fun((FSA, gleam@erlang@process:subject(FRZ)) -> FSB)}.
...
-type application_start_mode() :: normal |
    {takeover, gleam@otp@node:node_()} |
    {failover, gleam@otp@node:node_()}.

-type application_stop() :: any().

-spec start_child(child_spec(any(), FSI, FSJ), FSI) -> {ok, child(FSJ)} |
    {error, child_start_error()}.
...
-spec perform_instruction_for_child(
    FSW,
    instruction(),
    child_spec(any(), FSW, FSY),
    child(FSY)
) -> {ok, {child(FSY), instruction()}} | {error, child_start_error()}.
...
-spec supervisor(
    fun((FUF) -> {ok, gleam@erlang@process:subject(FUG)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(FUG, FUF, FUF).
supervisor(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.

-spec worker(
    fun((FUN) -> {ok, gleam@erlang@process:subject(FUO)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(FUO, FUN, FUN).
worker(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.

-spec returning(
    child_spec(FUV, FUW, any()),
    fun((FUW, gleam@erlang@process:subject(FUV)) -> FVC)
) -> child_spec(FUV, FUW, FVC).
returning(Child, Updater) ->
    {child_spec, erlang:element(2, Child), Updater}.

-spec init(spec(any(), FVH)) -> gleam@otp@actor:init_result(state(FVH), message()).
init(Spec) ->
...
-spec handle_exit(gleam@erlang@process:pid_(), state(FVN)) -> gleam@otp@actor:next(state(FVN)).
...
-spec loop(message(), state(FVR)) -> gleam@otp@actor:next(state(FVR)).
loop(Message, State) ->
    case Message of
        {exit, Exit_message} ->
...
-spec start_and_add_child(starter(FTP), child_spec(any(), FTP, FTS)) -> children(FTS).
...
-spec add(children(FTX), child_spec(any(), FTX, FUA)) -> children(FUA).
