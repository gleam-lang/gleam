...
use super::{
    ...
    are_path_dependency_manifests_unchanged, is_same_requirements, lookup_package,
    provide_git_package, provide_local_package, read_manifest_from_disc, remove_extra_packages,
    unlock_packages,
};
        ...
        let (requirements_changed, manifest_for_resolver) = match self.use_manifest {
            ...
            UseManifest::Yes => {
                ...
                if packages_to_update.is_empty()
                    && same_requirements
                    && are_path_dependency_manifests_unchanged(
                        &config_dependencies,
                        paths.root(),
                        paths,
                    )?
