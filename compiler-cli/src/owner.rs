...
use gleam_core::{Result, hex, paths::ProjectPaths};
use std::time::Instant;

pub fn transfer(paths: &ProjectPaths, new_owner_username_or_email: String) -> Result<()> {
...
Do you wish to transfer ownership of `{}` to {new_owner_username_or_email}?",
        config.name
    ))?;
