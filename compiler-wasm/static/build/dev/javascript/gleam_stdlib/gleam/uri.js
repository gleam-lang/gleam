...
import * as $function from "../gleam/function.js";
...
import * as $regex from "../gleam/regex.js";
...
function do_parse(uri_string) {
  ...
  let fragment$1 = (() => {
    ...
    let _pipe$2 = $result.then(_pipe$1, $string.pop_grapheme);
...
function regex_submatches(pattern, string) {
  ...
  let _pipe$4 = $result.then(_pipe$3, $list.first);
  ...
  while (true) {
    if (input.hasLength(0)) {
      ...
      let accumulator$1 = (() => {
        ...
        accumulator.hasLength(0)) {
          return toList([]);
        } else if (segment === ".." &&
        accumulator.atLeastLength(1)) {
...
export function origin(uri) {
  ...
  if (scheme instanceof Some && scheme[0] === "https" && isEqual(
  ...
  } else if (scheme instanceof Some && scheme[0] === "http" && isEqual(
    port,
    new Some(80)
  )) {
    let origin$1 = new Uri(
    ...
    return new Ok(to_string(origin$1));
  } else if (scheme instanceof Some && (scheme[0] === "http") || (scheme[0] === "https")) {
    let s = scheme[0];
    let origin$1 = new Uri(
      scheme,
      new None(),
...
export function merge(base, relative) {
  if (base instanceof Uri && base.scheme instanceof Some && base.host instanceof Some) {
    ...
    } else if (relative instanceof Uri && relative.scheme instanceof None && relative.host instanceof None) {
      let $ = (() => {
        let $1 = relative.path;
        if ($1 === "") {
