...
impl Telemetry for LogTelemetry {
    fn compiling_package(&self, name: &str, print_progress: bool) {
        if print_progress { tracing::info!("Compiling package: {}", name); }
    }
    ...
    fn downloading_package(&self, name: &str, print_progress: bool) {
        if print_progress { tracing::info!("Downloading package: {}", name); }
    }
    ...
    fn packages_downloaded(&self, _start: std::time::Instant, count: usize, print_progress: bool) {
        if print_progress { tracing::info!("Downloaded {} packages", count); }
    }
