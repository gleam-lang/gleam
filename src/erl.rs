...
fn record_update<'a>(
    spread: Cow<'a, TypedExpr>,
    args: Cow<'a, [TypedRecordUpdateArg]>,
    env: &mut Env,
) -> Document<'a> {
    use std::iter::once;

    args.into_cow_iter()
        .fold(expr(spread, env), |tuple_doc, arg| {
            ...
            let value = TypedRecordUpdateArg::butcher(arg).value;
            let value_doc = expr(value, env);
...
fn expr<'a>(expression: Cow<'a, TypedExpr>, env: &mut Env) -> Document<'a> {
    match TypedExpr::butcher(expression) {
        ...
        ButcheredTypedExpr::RecordUpdate { spread, args, .. } => record_update(spread, args, env),
