...
assertEqual(
  new BitArray(new Uint8Array([240, 159, 144, 0xa9, 0b11101010]), 39),
  toBitArray([
    new BitArray(new Uint8Array([240, 159, 144])),
    new BitArray(new Uint8Array([0xae]), 4),
    new BitArray(new Uint8Array([0x9f]), 7),
    new BitArray(new Uint8Array([0b01010000]), 4),
  ]),
);
...
assertThrows(
  "`BitArray.slice()` throw if start is less than zero",
  () => new BitArray(new Uint8Array([1])).slice(-1, 8),
);

assertThrows(
  "`BitArray.slice()` throw if start is greater than the bit size",
  () => new BitArray(new Uint8Array([1])).slice(9, 8),
);

assertThrows(
  "`BitArray.slice()` throw if end is before start",
  () => new BitArray(new Uint8Array([1])).slice(4, 2),
);

assertThrows(
  "`BitArray.slice()` throw if end is greater than the bit size",
...
function sliceToFloat(bytes, bitSize, start, end, isBigEndian) {
  return new BitArray(new Uint8Array(bytes), bitSize).sliceToFloat(
    start,
    end,
    isBigEndian,
  );
...
assertEqual(
  sliceToFloat([63, 240, 0, 0, 0, 0, 0, 0], 64, 0, 64, true),
  1.0,
);
assertEqual(
  sliceToFloat([0, 0, 0, 0, 0, 0, 240, 63], 64, 0, 64, false),
  1.0,
);
assertEqual(
  sliceToFloat([0xff, 0xc9, 0x74, 0x24, 0x00], 40, 8, 40, true),
  -1000000.0,
);
assertEqual(
  sliceToFloat([0x00, 0x24, 0x74, 0xc9], 32, 0, 32, false),
  -1000000.0,
);
assertEqual(
  sliceToFloat([112, 152, 127, 244, 0, 7, 192], 50, 11, 43, true),
  -511.25,
);
assertEqual(
  sliceToFloat([8, 0, 0, 0, 1, 129, 39, 103, 129, 254], 79, 7, 71, false),
  -5011.75,
);

// BitArray.sliceToInt()

function testSliceToInt(bytes, bitSize, start, end, isBigEndian, isSigned) {
  return new BitArray(new Uint8Array(bytes), bitSize).sliceToInt(
    start,
    end,
    isBigEndian,
    isSigned,
...
assertEqual(testSliceToInt([0b10011110], 8, 0, 4, true, false), 0b1001);
assertEqual(testSliceToInt([0b10011110], 8, 4, 8, true, false), 0b1110);
assertEqual(testSliceToInt([0b10011110], 8, 1, 6, true, true), 0b00111);
assertEqual(testSliceToInt([0b10011010], 8, 3, 8, true, true), -6);
assertEqual(testSliceToInt([0b10011010], 8, 0, 7, true, true), -51);
assertEqual(
  testSliceToInt([0b11001100, 0b00100000], 11, 1, 11, false, false),
  408,
);
assertEqual(testSliceToInt([0xb6, 0xe3], 16, 0, 12, true, false), 0xb6e);
assertEqual(testSliceToInt([0xb6, 0xe3], 16, 0, 12, false, false), 0xeb6);
assertEqual(
  testSliceToInt([0xff, 0xb6, 0xe3], 24, 8, 20, true, false),
  0xb6e,
);
assertEqual(
  testSliceToInt([0xff, 0xb6, 0xe3], 24, 20, 24, true, false),
  0x03,
);
assertEqual(
  testSliceToInt([0xff, 0xb6, 0xe3], 24, 8, 20, false, false),
  0xeb6,
);
assertEqual(testSliceToInt([0xa5, 0x6c, 0xaa], 24, 5, 18, true, false), 5554);
assertEqual(
  testSliceToInt([0xa5, 0x6c, 0xaa], 24, 5, 18, false, true),
  -3411,
);
assertEqual(testSliceToInt([1, 2, 3], 24, 0, 8, true, false), 1);
assertEqual(testSliceToInt([160, 2, 3], 24, 0, 8, false, true), -96);
assertEqual(testSliceToInt([1, 2, 3], 24, 0, 16, true, false), 258);
assertEqual(testSliceToInt([1, 2, 3], 24, 0, 16, false, false), 513);
assertEqual(testSliceToInt([1, 160, 3], 24, 0, 16, false, true), -24575);
assertEqual(testSliceToInt([160, 2, 3], 24, 0, 16, true, false), 40962);
assertEqual(testSliceToInt([3, 160, 2], 24, 8, 24, true, true), -24574);
assertEqual(
  testSliceToInt([146, 192, 70, 25, 128], 33, 1, 24, true, true),
  1_228_870,
);
assertEqual(testSliceToInt([255, 255, 255, 255], 32, 0, 32, false, true), -1);
assertEqual(
  testSliceToInt([217, 150, 209, 191, 0], 33, 1, 33, true, false),
  3_006_112_638,
);
assertEqual(
  testSliceToInt([146, 192, 70, 25, 128], 33, 1, 33, true, true),
  629_181_491,
);
assertEqual(
  testSliceToInt([251, 24, 47, 227, 128], 33, 1, 33, false, false),
  3_344_904_438,
);
assertEqual(
  testSliceToInt([240, 102, 91, 101, 128], 33, 0, 33, false, false),
  5_995_456_240,
);
assertEqual(
  testSliceToInt([231, 255, 255, 255, 254, 123], 48, 0, 40, true, true),
  -103_079_215_106,
);
assertEqual(
  testSliceToInt([0, 231, 255, 255, 253, 123, 17], 56, 1, 55, true, false),
  127_543_348_739_464,
);
assertEqual(
  testSliceToInt([142, 231, 255, 255, 253, 123, 17, 139], 64, 8, 62, false, true),
  -8425025061257241,
);
assertEqual(
  testSliceToInt([142, 231, 255, 255, 253, 123, 17, 139], 64, 7, 62, false, true),
  -8293899692933261,
);
assertEqual(
  testSliceToInt([142, 231, 255, 255, 253, 123, 17], 56, 8, 48, true, true),
  -103_079_215_749,
);
assertEqual(
  testSliceToInt([255, 255, 255, 255, 255, 255, 255], 56, 0, 56, true, true),
  -1,
);
assertEqual(
  testSliceToInt([0x00, 0xaa, 255, 255, 255, 255, 255], 56, 0, 56, true, false),
  0xaaffffffffff,
);
assertEqual(
  testSliceToInt([255, 255, 255, 255, 255, 0xaa, 0x00], 56, 0, 56, false, false),
  0xaaffffffffff,
);
assertEqual(
  testSliceToInt([255, 255, 255, 255, 255, 255, 255], 56, 0, 56, true, false),
  Number(0xfffffffffffffen),
);
assertEqual(testSliceToInt([0xfe, 0x3f], 16, 4, 12, true, false), 0xe3);
assertEqual(testSliceToInt([253, 94], 16, 3, 11, true, true), -22);
assertEqual(testSliceToInt([233, 164], 16, 3, 15, true, false), 1234);
assertEqual(testSliceToInt([250, 72], 16, 3, 15, false, false), 1234);
assertEqual(
  testSliceToInt([250, 72, 223, 189], 32, 7, 29, true, false),
  596983,
);
assertEqual(
  testSliceToInt(
