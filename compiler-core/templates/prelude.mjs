...
export class BitArray {
   ...
   *
   * If `bitOffset` is non-zero then there are unused bits in the first byte of
   * this buffer.
   *
   ...
   * bytes in the buffer are used.
   *
   * If no bit offset is specified it defaults to zero, i.e. there are no unused
   * high bits in the first byte of ths buffer.
   *
   ...
   *
   * If the final byte of a bit array is retrieved using this method and its bit
   * size is not a multiple of 8 then the content of the unused low bits are
   * undefined.
   *
   * @param {number} index
 ...
 * offset. No bounds checking is performed.
 *
 * @param {Uint8Array} buffer
 * @param {number} bitOffset
 * @param {number} index
...
export function bitArraySlice(bitArray, start, end) {
  ...
  // a bit array, i.e. only the bit offset or bit size need to be updated.
  let buffer;
