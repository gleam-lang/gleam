...
export class BitArray {
  ...
  /**
   * Returns this bit array's raw `Uint8Array` buffer.
   *
   * Code that reads this buffer directly must be able to correctly handle any
   * unaligned binary data it may contain. It is often preferable to use other
   * functions such as `iterateBytes()`, or `byteAt()` instead of working
   * directly with the internal buffer.
   *
   * @returns {Uint8Array}
   */
  get rawBuffer() {
  ...
  /**
   * Returns the number of bytes in this bit array. for bit arrays with a bit
   * size that is not a multiple of 8, the byte size value is rounded up.
   *
  ...
  /**
   * Returns an iterator over the bytes in this bit array. If this bit array is
   * not a whole number of bytes then `null` is returned.
   *
   * @returns {ArrayIterator<number> | null}
   */
  iterateBytes() {
    if (this.bitSize % 8 !== 0) {
      return null;
  ...
  /**
   * Returns the byte at the given index in this bit array. If the index is out
   * of range then an error is thrown.
   *
   * If this bit array's bit size is not a multiple of 8, the unused low bits of
   * its last byte are undefined.
   *
   * @param {number} index
   * @returns {number}
   */
   ...
   */
  get buffer() {
    printBitArrayDeprecationWarning(
      "buffer",
      "Use `BitArray.iterateBytes()`, `BitArray.byteSize`, " +
        "`BitArray.byteAt()`, or `BitArray.rawBuffer` instead.",
    );
  ...
  floatFromSlice(index, end, isBigEndian) {
    printBitArrayDeprecationWarning(
      "floatFromSlice",
      "Use `BitArray.sliceToFloat()` instead.",
    );
  ...
  intFromSlice(index, end, isBigEndian, isSigned) {
    printBitArrayDeprecationWarning(
      "intFromSlice",
      "Use `BitArray.sliceToInt()` instead.",
    );
...
function printBitArrayDeprecationWarning(functionName, message) {
  if (isBitArrayDeprecationMessagePrinted[functionName]) {
    return;
  }

  console.warn(`Deprecated function used`);
  console.warn(`  Name: BitArray.${functionName}()`);
  console.warn(`  Message: ${message}`);

  isBitArrayDeprecationMessagePrinted[functionName] = true;
}
