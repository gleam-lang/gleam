...
fn atom(value: &str) -> Document<'_> {
    if is_erlang_existing_function_name(value) {
        // Escape because of existing function name
        Document::String(format!("'gleam@{value}'"))
    } else if is_erlang_reserved_word(value) {
...
pub fn is_erlang_existing_function_name(name: &str) -> bool {
    matches!(name, "module_info")
}
