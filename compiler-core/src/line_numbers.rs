...
impl LineNumbers {
    ...
    // TODO: handle unicode characters that may be more than 1 byte in width
    /// 1 indexed line and character to byte index
    pub fn byte_index(&self, line: u32, character: u32) -> usize {
        match self.line_starts.get((line - 1) as usize) {
            Some(line_index) => *line_index + character as usize - 1,
            None => self.length,
        }
    }
}
...
"#;
    ...
    assert_eq!(line_numbers.byte_index(3, 1), 17);
}
