    ...
    pub fn find_node(&self, byte_index: u32) -> Option<Located<'_>> {
        match self {
            ...
            Self::ModuleSelect {
                ...
                let module_span = SrcSpan::new(location.start, location.start + (module_alias.len() as u32));

                if field_span.contains(byte_index) {
                    Some(self.into())
                }
                // Allow leniency to have cursor left of the '.' instead of directly on top of module name
