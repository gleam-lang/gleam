        ...
        .map(|level| {
            level
                ...
                .map(|function| match function {
                    CallGraphNode::Function(f) => f.name.map(|(_, name)| name).expect("Expected function name to be present"),
...
fn duplicate_external_function_name() {
    ...
    _ = parse_and_order(functions.as_slice(), [].as_slice()).expect_err("Expected parse_and_order to fail");
}
...
fn constants_and_functions() {
    ...
    assert_eq!(
        parse_and_order(functions.as_slice(), constants.as_slice()).expect("Expected parse_and_order to succeed"),
