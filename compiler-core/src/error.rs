...
is importing the dev module `{dev_module}`.

Dev modules are not included in production builds so application \
modules cannot import them. Perhaps move the `{dev_module}` \
