...
impl<'ast> ast::visit::Visit<'ast> for CollapseNestedCase<'ast> {
    fn visit_typed_clause(&mut self, clause: &'ast ast::TypedClause) {
        ...
        // We can't have any alternative patterns...
        if alternative_patterns.is_empty()
            // ...and the then clause must be a single case expression...
            && let Some(TypedExpr::Case {
                subjects, clauses, ..
            }) = then.expression_flattening_block()
            // ...that is matching a single variable...
            && let [TypedExpr::Var { name, .. }] = subjects.as_slice()
            // ...and that variable must be one the variables we brought into
            // scope in this branch.
            && let Some((variable_name, variable_location)) = pattern
                .iter()
                ...
                .find(|(variable, _)| variable == name)
        {
            ...
            // ```
            //
            let pattern_location = pattern
                .first()
                .map_or(*location, |first| first.location())
                .merge(&pattern.last().map_or(*location, |end| end.location()));

            let valid_location = match subjects.last() {
                Some(last_subject) => pattern_location.merge(&last_subject.location()),
                None => pattern_location,
            };
            let valid_range = self.edits.src_span_to_lsp_range(valid_location);

            if within(self.params.range, valid_range) {
                ...
                // We're done, there's no need to keep exploring as we know the cursor
                // is over this pattern and it can't be over any other one!
                return;
            }
        };
