...
{labelled}"
    ...
    pub(crate) fn signature_help(
        ...
        self.respond(|this| {
            let Some(module) = this.module_for_uri(&position_params.text_document.uri) else {
                return Ok(None);
            };

            match this.node_at_position(&params.text_document_position_params) {
                Some((_lines, Located::Expression { expression, .. })) => Ok(
                    signature_help::for_expression(expression, &module.ast.names),
                ),
