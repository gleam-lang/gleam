...
fn format_code_action_file_operations<'a>(actions: Vec<lsp_types::CodeAction>) -> String {
    actions
        .into_iter()
        .flat_map(|action| {
            let DocumentChanges::Operations(operations) = action
                .edit
                .expect("No workspace edit for action")
                .document_changes
                .expect("No document changes for action")
            else {
                panic!("No document change operations for action")
            };
