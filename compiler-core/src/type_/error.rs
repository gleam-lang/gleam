...
pub enum VariableSyntax {
    ...
    /// Contained location is the location of assignment from the end of the pattern
    /// to the end of assignment itself, so it includes the `as` keyword.
    /// For example,
    ///
    /// ```gleam
    /// case x {
