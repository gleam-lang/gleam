use super::*;
use crate::ast::{TodoKind, UntypedExpr};
...
#[test]
fn duplicate_qualified_imports_test() {
    let mut warnings = vec![];
    let ids = UniqueIdGenerator::new();
    let mut modules = im::HashMap::new();
    ...
    let (mut ast, _) = crate::parse::parse_module("pub fn bar() { 1 }").expect("syntax error");
    ast.name = vec!["gleam".to_string(), "foo".to_string()];
    let module = infer_module(
        Target::Erlang,
        &ids,
        ast,
    ...
    let _ = modules.insert(
        vec!["gleam".to_string(), "foo".to_string()].join("/"),
        module.type_info,
    );

    let (mut ast, _) = crate::parse::parse_module("pub fn bar() { 2 }").expect("syntax error");
    ast.name = vec!["gleam2".to_string(), "foo".to_string()];
    let module = infer_module(
        Target::Erlang,
        &ids,
        ast,
    ...
    .expect("should successfully infer");
    let _ = modules.insert(
        vec!["gleam2".to_string(), "foo".to_string()].join("/"),
        module.type_info,
    );

    let (mut ast, _) = crate::parse::parse_module(
        "
    import gleam/foo
    import gleam2/foo
    ...
    assert_eq!(
        Error::DuplicateImport {
            location: SrcSpan { start: 33, end: 43 },
            previous_location: SrcSpan { start: 12, end: 21 },
            name: "gleam2/foo".to_string()
        },
        infer_result.err().unwrap()
    );
}
