...
impl Manifest {
    ...
    pub fn to_toml(&self, path_resolver: impl FnOnce(&Utf8Path) -> Utf8PathBuf + Clone) -> String {
...
",
        ...
        } in packages.iter().sorted_by(|a, b| a.name.cmp(&b.name))
        {
            ...
            match source {
                ...
                ManifestPackageSource::Local { path } => {
                    ...
                    buffer.push_str(path_resolver(path).as_str());
                    buffer.push('"');
                }
            };
        ...
        for (name, requirement) in requirements.iter().sorted_by(|a, b| a.0.cmp(b.0)) {
            buffer.push_str(name);
            buffer.push_str(" = ");
            buffer.push_str(&requirement.to_toml(path_resolver.clone()));
            buffer.push('\n');
        }
...
fn manifest_toml_format() {
    ...
    let test_resolver = |path: &Utf8Path| {
        pathdiff::diff_utf8_paths(path, "/home/louis/packages/curr_dir").map(|p| {
            Utf8PathBuf::from(format!("../{}", p.as_str()))
        }).unwrap_or(path.into())
    };
    let buffer = manifest.to_toml(test_resolver);
    assert_eq!(
        buffer,
