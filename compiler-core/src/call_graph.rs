...
impl<'a> Visit<'a> for MutualRecursionGraphBuilder {
    ...
    fn visit_typed_expr(&mut self, expr: &'a ast::TypedExpr) {
        ...
        match expr {
            ...
            ast::TypedExpr::Call { fun, .. } => match fun.as_ref() {
                // Here we might be calling a top level function and so have a
                // mutually recursive call!
                ast::TypedExpr::Var { name, .. } => self.referenced(name),

                // Here we're calling something that is not simply a name, so
                // we can ignore it for the sake of finding mutually recursive
                // functions.
                _ => (),
            },
