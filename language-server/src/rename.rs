...
fn rename_same_named_prelude_import_reference(
    ...
    let mut found_renamed_import = false;
    let mut previous_import_location = None;
    let mut next_import_location = None;
    for import_location in imported_type_locations
        .chain(imported_value_locations)
        .sorted_by_key(|location| location.start)
    {
        if found_renamed_import {
            next_import_location = Some(import_location);
            break;
        }

        if import_location == location {
            found_renamed_import = true;
            continue;
        }

        previous_import_location = Some(import_location);
    }

    match (previous_import_location, next_import_location) {
        // If there's an import following the renamed import, we need to delete
